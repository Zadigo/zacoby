...
class DomElement(DOMElementMixin):
    ...
    def __hash__(self):
        return hash((self.ELEMENT_VALUE, self.ELEMENT_ID))
    
    def __eq__(self, value):
        return value == self.ELEMENT_VALUE or value == self.ELEMENT_ID
        
    @classmethod
    def as_class(cls, remote_connection, response):
        instance = cls()
        instance._remote_connection = remote_connection
        instance.parse_response(response)                
        return instance

    def parse_response(self, response):
        self.ELEMENT_ID = 'Kendall'
        self.ELEMENT_VALUE = 'ieeifnapecneeubzoeeczeno'
