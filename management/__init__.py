...
def collect_commands():
    ...
    commands_path = list(os.walk(os.path.join(lazy_settings.GLOBAL_ZACOBY_PATH, 'management', 'commands')))
...
def load_command_class(name: str) -> Callable:
    ...
    for path in   paths:
        ...
        try:
            module = import_module(f'zacoby.management.commands.{name}')
        except:
            raise ImportError(f"Could not import module at {path} from the Zacoby commands directory.")
...
class Utility:
    ...
    def __init__(self):
        ...
        for path in modules_paths:
            ...
            try:
                module_obj = import_module(f'zacoby.management.commands.{true_name}')
    ...
    def _parse_incoming_commands(self, args: list):
        if len(args) <= 1:
            raise ValueError(('You called manage.py or python -m zacoby '
